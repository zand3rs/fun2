...
/**
#----------------------------------------------------------------------------------
#	@Filename	:	BulkFunDeactivation.class.php
...
//class
class BulkFunDeactivation
{

	//constructor
	function BulkFunDeactivation()
	{
	...
	function SaveList($data=null,$grpid=null)
	{
		...
		$chkgsm = 0;
			...
			$chkgsm = 3;
			$errReason = "OFW SIM is not a PRAPAID No.";
		} else if ($this->isBlacklisted($vgsmnum1) >= 1) {
			$chkgsm = 4;
			$errReason = "OFW SIM is Blacklisted";
		} else if ($this->chkActiveFun($vgsmnum1,$vgsmnum2) == 0) {
			$chkgsm = 5;
			$errReason = "Link not exists";
			...
			$chkgsm = 3;
			$errReason = "LOCAL SIM is not a PRAPAID No.";
		} else if ($this->isBlacklisted($vgsmnum2) >= 1) {
			$chkgsm = 4;
			$errReason = "LOCAL SIM is Blacklisted";
		} else if ($this->chkPendingRequest($vgsmnum1,$vgsmnum2,'CANCEL') >= 1) {
			$chkgsm = 6;
			$errReason = "With Pending Request";
		} else if ($this->chkPendingRequest($vgsmnum1,$vgsmnum2,'UNREG') >= 1) {
			$chkgsm = 6;
			$errReason = "With Pending Request";
		} 
		...
		{
			//new profile
			$sql       = "update sim_activation_links ".
			             "set    status = 'INACTIVE', ".
			             ...
			             "       link_cmd ='BULK' ".
			             "where  msisdn = $vgsmnum1 ".
			             "and    link_to = $vgsmnum2 ";
 			//exect
			$data      = $myOraDb->xQuery($sql);
		}
		else{
			//new profile
			$sql       = "INSERT INTO SIM_DEACT_LINKS_ERR_BULK ( OFW_SIM, LOCAL_SIM, REASON, REMARKS, BULK_GRP_ID, CREATED_BY, DT_CREATED, TRAN_DT ) ". 
	...
	function getList($grp_id=null)
	{
		...
		//select values for dynamic SQL passing
		$sql  = "  SELECT * FROM ( SELECT ROWNUM MROW, b.* FROM ( SELECT 0 ID, OFW_SIM, LOCAL_SIM, CREATED_BY, DT_CREATED, BULK_GRP_ID, REASON REASON  FROM SIM_DEACT_LINKS_ERR_BULK ".
	...
	function chkActiveFun($c1='',$c2='')
	{
		...
		//update profile
		$sql       = " SELECT sf_is_link_active($c1, $c2) TOTAL FROM DUAL";
		$stmt = OCIParse($conn,$sql);
		$totalrows = 0;	
		OCIDefineByName($stmt,"TOTAL",$totalrows);		
		OCIExecute($stmt);
