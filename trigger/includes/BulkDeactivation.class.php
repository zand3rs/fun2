...
/**
#----------------------------------------------------------------------------------
#	@Filename	:	BulkDeactivation.class.php
#	@Description:	encapsulator for table Manual_Enrollment_Log
#
#
#	@Date		:	2009-05-25
#	@Ver		:	ver1.1
...
//class
class BulkDeactivation
{

	//constructor
	function BulkDeactivation()
	{
	...
	function SaveList($data=null,$grpid=null)
	{
		...
		$this->debug("SaveList() a#: $vgsmnum, $vname, $vcontact_number, $vremarks, $grpid");
		...
		} else if ($this->chkRoamer($vgsmnum) == 0){
			$chkgsm = 5;
			$errReason = "No Active Roaming";
		} else if ($this->chkPendingRequest($vgsmnum,'DEACTIVATION') >= 1){
			$chkgsm = 6;
		...
		if ($chkgsm == 0) 
		{
			...
			$sql       = "INSERT INTO MANUAL_ENROLLMENT_LOG ( ID, MSISDN, ENROLLMENT_TYPE, BULK_UPLOAD, TX_DATE, STATUS, CS_USERNAME, DT_CREATED, subs_name, subs_contact_no, remarks, bulk_grp_id, min_bal, breathing_period, customer_type, request_origin ) ". 
				     "VALUES (manual_enrollment_log_seq.nextval, $vgsmnum, 'DEACTIVATION', 1, trunc(sysdate), 0, $vCreated_by, sysdate, $vname, $vcontact_number, $vremarks, '$grpid', 0, 0, 'TRIGGER', 'WEB') ";
			...
			//new profile
			$sql       = "INSERT INTO BULK_DEACTIVATION_ERR_LOG ( MSISDN, TX_DATE, SUBS_NAME, SUBS_CONTACT_NO, REMARKS, BULK_GRP_ID, CS_USERNAME, DT_CREATED, CS_REASON ) ". 
	...
	function getList($grp_id=null)
	{
		...
		{
			case 'msisdn':
				$orderby = " ORDER BY  msisdn $ascdesc";
				break;
			case 'subs_name':
				$orderby = " ORDER BY  subs_name $ascdesc";
				break;
			case 'subs_group':
				$orderby = " ORDER BY  subs_group $ascdesc";
				break;
			case 'subs_contact_no':
				$orderby = " ORDER BY  subs_contact_no $ascdesc";
				break;
			default:	
		...
		//select values for dynamic SQL passing
		$sql  = "  SELECT * FROM ( SELECT ROWNUM MROW, b.* FROM ( SELECT 0 ID, MSISDN, TX_DATE, CS_USERNAME, DT_CREATED, SUBS_NAME, SUBS_CONTACT_NO, REMARKS, BULK_GRP_ID, CS_REASON REASON  FROM BULK_DEACTIVATION_ERR_LOG ".
