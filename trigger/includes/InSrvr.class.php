...
/**
#----------------------------------------------------------------------------------
#	@Filename	:	InSrvr.class.php
#	@Description:	encapsulator for table IN_SERVERS
#
#
#	@Date		:	2009-05-25
#	@Ver		:	ver1.1
...
define('CSV_HEADER_INSVR_REPORT',  "SERVER_IP, SERVER_PORT, SERVER_NAME, USERNAME, PASSWORD, TIMEOUT, RETRY_CNT, SVC_NAME, EXPIRY, STATUS, CREATED BY, DATE, MODIFIED BY, DATE");
...
class InSrvr
{
	...
	function Get($id='')
	{
		...
		//format the insert-parameters
		$sql     = " SELECT  id, server_ip, server_port, server_name, username, password, timeout, retry_cnt, svc_name, expiry, status FROM IN_SERVERS WHERE ID=$id ";
	...
	function Save($data=null)
	{
		...
		//new profile
		$sql       = " INSERT INTO IN_SERVERS ($kbuff, DT_CREATED )  VALUES ($vbuff, sysdate )";
		...
		$gEventLogger->LogEvent(__CLASS__, "NEW IN Server: $mbuff");
	...
	function Update($data=null)
	{
		...
		//new profile
		$sql       = " UPDATE IN_SERVERS SET DT_UPDATED=Sysdate, $kbuff WHERE ID=$id ";
		...
		$gEventLogger->LogEvent(__CLASS__, "UPDATE IN Server: $mbuff");
	...
	function Delete($id=0)
	{
		...
		$id        = intval($id);
		//update profile
		$sql       = " DELETE FROM IN_SERVERS WHERE ID=$id ";
		...
		$gEventLogger->LogEvent(__CLASS__, "DELETE IN Server: ID=$id");
	...
	function IfExists($c1='',$id=0,$ok=0)
	{
		...
		//update profile
		$sql       = ($id>0 and $ok) ? 
			" SELECT  COUNT(1) as TOTAL FROM IN_SERVERS WHERE SERVER_NAME='$c1' AND ID!=$id " :
			" SELECT  COUNT(1) as TOTAL FROM IN_SERVERS WHERE SERVER_NAME='$c1' ";
	...
	function getTableFields()
	{
		...
		$table_fields= array(
				...
				array('name' => 'svc_name'     ,'title'=>'Svc Name',      'type' => 'text',    'max'=> 64,   'data_type'=> 'string', 'value' => $_POST['svc_name'   ] ),
				...
				array('name' => 'expiry'       ,'title'=>'Expiry',        'type' => 'text',    'max'=> 12,   'data_type'=> 'string', 'value' => $_POST['expiry'     ] ),
	...
	function getTableFields4Update($id="")
	{
		...
		$table_fields= array(
				...
				array('name' => 'svc_name'     ,'title'=>'Svc Name',      'type' => 'text',    'max'=> 64,   'data_type'=> 'string', 'value' => $post['svc_name'   ] ),
				...
				array('name' => 'expiry'       ,'title'=>'Expiry',        'type' => 'text',    'max'=> 12,   'data_type'=> 'string', 'value' => $post['expiry'     ] ),
	...
	function getList($extravars=null)
	{
		...
		$sql  = " SELECT COUNT(1) AS COUNT FROM IN_SERVERS $xwhere";
		...
		//event
		//$gEventLogger->LogEvent(__CLASS__, "getInSrvrList : [WHERE=>'$xwhere'] [ TOTAL=>'$totalrows'] ");
		...
		if ((strlen($temp_file)>0) && ($temp_file==1)) {
			...
			//select values for dynamic SQL passing
			$sql  = "  SELECT * FROM ( SELECT ROWNUM MROW, b.* FROM ( SELECT id, server_ip, server_port, server_name, username, password, timeout, retry_cnt, svc_name, expiry, status FROM IN_SERVERS ".
				"  $xwhere $orderby ) b ) WHERE MROW >=$slimit AND MROW < $elimit";
                	
			//Parse SQL passed
			$stmt = OCIParse($conn,$sql);
                	
			$this->debug("getInSrvrList() : get_all_rows[$sql / $stmt / $totalrows]");
		...
		//event
		//$gEventLogger->LogEvent(__CLASS__, "getInSrvrList(#b) : [ total=>'$totalrows'] ");
	...
	function downloadList($extravars=null)
	{
		...
		//select values for dynamic SQL passing
		$sql  = "  SELECT * FROM ( SELECT ROWNUM MROW, b.* FROM ( SELECT id, server_ip, server_port, server_name, username, password, timeout, retry_cnt, svc_name, expiry, status, created_by, ".
		...
		//close csv
                ...
                file_save($csvfile,"List of IN Servers\nReport Generation: $rep_gen_dt\n\n".CSV_HEADER_INSVR_REPORT."\n\n".$csvcont);
