...
/**
#----------------------------------------------------------------------------------
#	@Filename	:	HlrSrvrMapping.class.php
#	@Description    :	encapsulator for table HLR_SERVER_MAPPING
#
#
#	@Date		:	2009-05-25
#	@Ver		:	ver1.1
...
define('CSV_HEADER_HLRSVRMAPPING_REPORT',  "SERVER_NAME, MSISDN_FR, MSISDN_TO, STATUS, CREATED BY, DATE, MODIFIED BY, DATE");

//class
class HlrSrvrMapping
{

	//constructor
	function HlrSrvrMapping()
	{
	...
	function Get($id='')
	{
		...
		//format the insert-parameters
		$sql     = " SELECT  id, server_name, msisdn_fr, msisdn_to, status FROM HLR_SERVER_MAPPING WHERE ID=$id ";
	...
	function Save($data=null)
	{
		...
		//new profile
		$sql       = " INSERT INTO HLR_SERVER_MAPPING ($kbuff, DT_CREATED )  VALUES ($vbuff, sysdate )";
	...
	function Update($data=null)
	{
		...
		//new profile
		$sql       = " UPDATE HLR_SERVER_MAPPING SET DT_UPDATED=Sysdate, $kbuff WHERE ID=$id ";
		...
		$gEventLogger->LogEvent(__CLASS__, "UPDATE HLR Mapping: $mbuff");
	...
	function Delete($id=0)
	{
		...
		$id        = intval($id);
		//update profile
		$sql       = " DELETE FROM HLR_SERVER_MAPPING WHERE ID=$id ";
		...
		$gEventLogger->LogEvent(__CLASS__, "DELETE HLR Mapping: ID=$id");
	...
	function IfExists($c1='', $c2='', $id=0, $ok=0)
	{
		...
		//update profile
		$sql       = ($id>0 and $ok) ? 
			" SELECT  COUNT(1) as TOTAL FROM HLR_SERVER_MAPPING WHERE SERVER_NAME='$c1' and MSISDN_FR = $c2 AND ID!=$id " :
			" SELECT  COUNT(1) as TOTAL FROM HLR_SERVER_MAPPING WHERE SERVER_NAME='$c1' and MSISDN_FR = $c2 ";
	...
	function IfOverlapRange($c1='', $c2='', $c3='', $id=0)
	{
		...
		$sql       = " SELECT SUM(TOTAL) TOTAL FROM (".
			     " SELECT COUNT(1) as TOTAL FROM HLR_SERVER_MAPPING WHERE SERVER_NAME = '$c1' AND $c2 BETWEEN MSISDN_FR AND MSISDN_TO AND ID!=$id".
			     " UNION ".
			     " SELECT COUNT(1) as TOTAL FROM HLR_SERVER_MAPPING WHERE SERVER_NAME = '$c1' AND $c3 BETWEEN MSISDN_FR AND MSISDN_TO AND ID!=$id".
			     " UNION ".
			     " SELECT COUNT(1) as TOTAL FROM HLR_SERVER_MAPPING WHERE SERVER_NAME = '$c1' AND MSISDN_FR BETWEEN $c2 AND $c3 AND ID!=$id".
			     " UNION ".
			     " SELECT COUNT(1) as TOTAL FROM HLR_SERVER_MAPPING WHERE SERVER_NAME = '$c1' AND MSISDN_TO BETWEEN $c2 AND $c3 AND ID!=$id)";
	...
	function getList($extravars=null)
	{
		...
		$sql  = " SELECT COUNT(1) AS COUNT FROM HLR_SERVER_MAPPING $xwhere";
		...
		//event
		//$gEventLogger->LogEvent(__CLASS__, "getHlrSrvrMappingList : [WHERE=>'$xwhere'] [ TOTAL=>'$totalrows'] ");
		...
		if ((strlen($temp_file)>0) && ($temp_file==1)) {
			...
			//select values for dynamic SQL passing
			$sql  = "  SELECT * FROM ( SELECT ROWNUM MROW, b.* FROM ( SELECT id, server_name, msisdn_fr, msisdn_to, status FROM HLR_SERVER_MAPPING ".
				"  $xwhere $orderby ) b ) WHERE MROW >=$slimit AND MROW < $elimit";
                	
			//Parse SQL passed
			$stmt = OCIParse($conn,$sql);
                	
			$this->debug("getHlrSrvrMappingList() : get_all_rows[$sql / $stmt / $totalrows]");
		...
		//event
		//$gEventLogger->LogEvent(__CLASS__, "getHlrSrvrMappingList(#b) : [ total=>'$totalrows'] ");
	...
	function downloadList($extravars=null)
	{
		...
		//orderby
		$ascdesc = " ASC ";	
		$orderby = " ORDER BY  server_name, msisdn_fr $ascdesc";
