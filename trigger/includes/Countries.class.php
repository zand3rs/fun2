...
/**
#----------------------------------------------------------------------------------
#	@Filename	:	Countries.class.php
#	@Description:	encapsulator for table COUNTRIES
#
#
#	@Date		:	2009-05-25
#	@Ver		:	ver1.1
...
define('CSV_HEADER_COUNTRIES_REPORT',  "COUNTRY_CODE, COUNTRY_NAME, ZONE_ID, CHARGE_DESC, STATUS, CREATED BY, DATE, MODIFIED BY, DATE");
...
class Countries
{
	...
	function Get($id='')
	{
		...
		//format the insert-parameters
		$sql     = " SELECT id, country_code, country_name, zone_id, charge_desc, status FROM COUNTRIES WHERE ID=$id ";
		...
		OCIDefineByName($stmt, 'COUNTRY_NAME' , $P_COUNTRY_NAME );
		OCIDefineByName($stmt, 'ZONE_ID'      , $P_ZONE_ID      );
		OCIDefineByName($stmt, 'CHARGE_DESC'  , $P_CHARGE_DESC  );
	...
	function CheckFields($data=null,$ok=0)
	{
		...
		//user 
		$ret = $this->IfExists($data['country_code'], $data['id'], $ok);
		if($ret > 0)
		{
			$err['error'] = true;
		...
		//usr
		if(strlen(trim($data['country_code'])) <= 0)
		{
			$err['error'] = true;
	...
	function Save($data=null)
	{
		...
		//new profile
		$sql       = " INSERT INTO COUNTRIES ($kbuff, DT_CREATED )  VALUES ($vbuff, sysdate )";
		...
		$gEventLogger->LogEvent(__CLASS__, "NEW COUNTRY: $mbuff");
	...
	function Update($data=null)
	{
		...
		//new profile
		$sql       = " UPDATE COUNTRIES SET DT_UPDATED=Sysdate, $kbuff WHERE ID=$id ";
		...
		$gEventLogger->LogEvent(__CLASS__, "UPDATE COUNTRY: $mbuff");
	...
	function Delete($id=0)
	{
		...
		$gEventLogger->LogEvent(__CLASS__, "DELETE COUNTRY: ID=$id");
	...
	function IfExists($c1='',$id=0,$ok=0)
	{
		...
		//update profile
		$sql       = ($id>0 and $ok) ? 
			" SELECT  COUNT(1) as TOTAL FROM COUNTRIES WHERE COUNTRY_CODE='$c1' AND ID!=$id " :
			" SELECT  COUNT(1) as TOTAL FROM COUNTRIES WHERE COUNTRY_CODE='$c1' ";
	...
	function getTableFields()
	{
		...
		$opts_zone   = $gSelectOptionsMaster->getZones();

		$table_fields= array(
				array('name' => 'country_code',   'title'=>'Country Code',  'type' => 'text',    'max'=> 30,   'data_type'=> 'string', 'value' => $_POST['country_code'] ),
				array('name' => 'country_name',   'title'=>'Country Name',  'type' => 'text',    'max'=> 30,   'data_type'=> 'string', 'value' => $_POST['country_name'] ),
				array('name' => 'zone_id' ,       'title'=>'Zone',          'type' => 'select',  'max'=> 12,   'data_type'=> 'string', 'value' => $_POST['zone_id'], 'select_options'=>$opts_zone, 'select_selected' => $_POST['zone_id'] ),
				array('name' => 'charge_desc' ,   'title'=>'Charge',        'type' => 'textarea','max'=> 255,  'data_type'=> 'string', 'value' => $_POST['charge_desc'], 'rows'=>3, 'cols'=>70),
	...
	function getTableFields4Update($id="")
	{
		...
		$table_fields= array(
				array('name' => 'country_code',   'title'=>'Country Code',  'type' => 'text',    'max'=> 30,    'data_type'=> 'string', 'value' => $post['country_code'] ),
				array('name' => 'country_name',   'title'=>'Country Name',  'type' => 'text',    'max'=> 30,    'data_type'=> 'string', 'value' => $post['country_name'] ),
				array('name' => 'zone_id' ,       'title'=>'Zone',          'type' => 'select',  'max'=> 12,    'data_type'=> 'string', 'value' => $post['zone_id'], 'select_options'=>$opts_zone, 'select_selected' => $post['zone_id'] ),
				array('name' => 'charge_desc' ,   'title'=>'Charge',        'type' => 'textarea','max'=> 255,  'data_type'=> 'string', 'value' => $post['charge_desc'], 'rows'=>3, 'cols'=>70),
	...
	function getList($extravars=null)
	{
		...
		if (strlen($temp_name) > 0) { 
			$xwhere .= " AND country_name like '$temp_name%' "; 
			$xwherelog .= " Country Name=$temp_name";
			$filters++; 
		}

		if ((strlen($temp_zone) > 0)  && ($temp_zone <> '0'))  { 
			$xwhere .= " AND zone_id like '$temp_zone%' "; 
		...
		$sql  = " SELECT COUNT(1) AS COUNT FROM COUNTRIES $xwhere";
		...
		//event
		//$gEventLogger->LogEvent(__CLASS__, "getCountriesList : [WHERE=>'$xwhere'] [ TOTAL=>'$totalrows'] ");
		...
		if ((strlen($temp_file)>0) && ($temp_file==1)) {
			...
			switch($sortedby)
			{
				case 'country_code':
					...
					break;
				case 'country_name':
					$orderby = " ORDER BY  country_name $ascdesc";
					break;
				case 'zone_id':
			...
			//select values for dynamic SQL passing
			$sql  = "  SELECT * FROM ( SELECT ROWNUM MROW, b.* FROM ( SELECT id, country_code, country_name, zone_id, charge_desc, status FROM COUNTRIES ".
				"  $xwhere $orderby ) b ) WHERE MROW >=$slimit AND MROW < $elimit";
                	
			//Parse SQL passed
			$stmt = OCIParse($conn,$sql);
                	
			$this->debug("getCountriesList() : get_all_rows[$sql / $stmt / $totalrows]");
			...
			while (OCIFetch($stmt))
			{
				...
				$alldata[$idx]['country_name']    = $p_country_name ;
				$alldata[$idx]['zone_id'     ]    = $p_zone_id      ;
				$alldata[$idx]['charge_desc' ]    = $p_charge_desc  ;
		...
		//event
		//$gEventLogger->LogEvent(__CLASS__, "getCountriesList(#b) : [ total=>'$totalrows'] ");
	...
	function downloadList($extravars=null)
	{
		...
		if ((strlen($temp_zone) > 0)  && ($temp_stat > 0))  { 
			$xwhere .= " AND zone like '$temp_zone%' "; 
			$filters++; 
		}
		...
		//select values for dynamic SQL passing
		$sql  = "  SELECT * FROM ( SELECT ROWNUM MROW, b.* FROM ( SELECT id, country_code, country_name, zone_id, charge_desc, status, created_by, ".
		...
		while (OCIFetch($stmt))
		{
			...
			$csvdata[] = $p_country_name ; 
			$csvdata[] = $p_zone_id      ; 
			$csvdata[] = $p_charge_desc  ; 
			$csvdata[] = $p_status       ; 
		...
		//close csv
                ...
                file_save($csvfile,"List of Countries\nReport Generation: $rep_gen_dt\n\n".CSV_HEADER_COUNTRIES_REPORT."\n\n".$csvcont);
