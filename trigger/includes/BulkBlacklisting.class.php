...
/**
#----------------------------------------------------------------------------------
#	@Filename	:	BulkBlacklisting.class.php
#	@Description:	encapsulator for table BulkBlacklisting_log
#
#
#	@Date		:	2009-05-25
#	@Ver		:	ver1.1
...
//class
class BulkBlacklisting
{

	//constructor
	function BulkBlacklisting()
	{
	...
	function SaveList($data=null,$grpid=null)
	{
		...
		if ($chkgsm == 0) 
		{
			...
			$data      = $myOraDb->xQuery($sql);
                	...
                	$sql       = "INSERT INTO BLACKLISTED_LOG ( MSISDN, TRAN_DT, CREATED_BY, DT_CREATED, REMARKS, bulk_grp_id ) ".
				     "VALUES ( $vgsmnum, trunc(sysdate), $vCreated_by, sysdate, $vremarks, '$grpid' ) ";
			$data      = $myOraDb->xQuery($sql);
		} else {
                        $sql       = "INSERT INTO BLACKLISTED_ERR_LOG ( MSISDN, TRAN_DT, CREATED_BY, DT_CREATED, REMARKS, bulk_grp_id, REASON) ".
	...
	function getList($grp_id=null)
	{
		...
		$sql  = " SELECT COUNT(1) AS COUNT FROM BLACKLISTED_LOG $xwhere";
		...
		//select values for dynamic SQL passing
		$sql  = "  SELECT * FROM ( SELECT ROWNUM MROW, b.* FROM ( SELECT MSISDN, to_char(TRAN_DT, 'YYYY-MM-DD') TX_DATE, to_char(DT_CREATED, 'YYYY-MM-DD HH24:MI:SS') DT_CREATED, REMARKS, REASON FROM BLACKLISTED_ERR_LOG ".
