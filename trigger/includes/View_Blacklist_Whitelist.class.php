...
/**
#----------------------------------------------------------------------------------
#	@Filename	:	View_Blacklist_Whitelist.class.php
...
define('CSV_HEADER_SUBS_REPORT',  "msisdn, action, remarks, date created, created by");

//class
class View_Blacklist_Whitelist
{

	var $_Data;

	//constructor
	function View_Blacklist_Whitelist()
	{
		//reset
		$this->_Data = null;
	}
	...
	function ListSubscribers($extravars, $q_sdate,$q_edate,$q_msisdn )
	{
		...
		$sql  = " SELECT COUNT(1) AS COUNT FROM BLACKLISTED_WHITELISTED_LOG a $mwhere ";
		...
		$this->debug("ListBlacklisted() : get_max_row[$sql / $stmt / $totalrows]");
		...
		//event
		//$gEventLogger->LogEvent(__CLASS__, "ListBlacklisted : [WHERE=>'$mwhere'] [ TOTAL=>'$totalrows'] ");
		...
		$mselect = "  SELECT msisdn,to_char(dt_created,'YYYY-MM-DD HH24:MI:SS') dt_created,".
		           "        action, created_by, remarks ".
		           "  FROM BLACKLISTED_WHITELISTED_LOG  a $mwhere $orderby ";
		...
		$this->debug("ListBlacklisted() : get_all_rows[$sql / $stmt / $totalrows]");


		//bind
		OCIDefineByName($stmt, "MSISDN"     , $p_msisdn       );
		OCIDefineByName($stmt, "ACTION"     , $p_action       );
		...
		//event
		//$gEventLogger->LogEvent(__CLASS__, "ListBlacklisted(#b) : [ total=>'$idx'] ");
