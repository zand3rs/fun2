	...
	else
	{
		...
		if(isset($_POST))
		{
			...
			if(null != $allfields)
			{
			   ...
			   if($ret['error'])
			   {
				$gSmarty->assign('error_msg', "Update IN Mapping Failed.");
			   }
			   else
			   {
				//lookup
				...
				$gSmarty->assign('error_msg', "Update IN Mapping Successfully Completed.");
...
else
{
	...
	if($postf == null)
	{
	   $gSmarty->assign('error_msg', UPDATE_IN_SEVER_MAPPING_FAILED_NO_RECORD_FOUND); 
	}
...
$gSmarty->assign('form_title',    "IN Server Mapping Maintenance");
$gSmarty->assign('form_action',   "update_insrvr_mapping.php");
