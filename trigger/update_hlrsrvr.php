	...
	else
	{
		...
		if(isset($_POST))
		{
			...
			if(null != $allfields)
			{
			   ...
			   if($ret['error'])
			   {
				$gSmarty->assign('error_msg', "Update HLR Server Failed.");
			   }
			   else
			   {
				//lookup
				...
				$gSmarty->assign('error_msg', "Update HLR Server Successfully Completed.");
				$gSmarty->display('list_hlrsrvr.tpl');
				exit;
			   }
			}
		}
...
else
{
	...
	if($postf == null)
	{
	   $gSmarty->assign('error_msg', UPDATE_HLR_SEVER_FAILED_NO_RECORD_FOUND); 
	}
...
$gSmarty->assign('form_title',    "HLR Server Maintenance");
$gSmarty->assign('form_action',   "update_hlrsrvr.php");
