...
{include file="menu.tpl"}
	...
	<form name="mainform" method="post" action="{$form_action}">
		...
		{foreach from=$master_fields item=master_field}
			...
			<td class="child">
				{if $master_field.type == "text"}
					<input type="{$master_field.type}" name="{$master_field.name}" value="{$master_field.value|escape:"html"}" style="width:{$master_field.width}px" maxlength="{$master_field.max}"/>
				...
				{elseif $master_field.type == "textarea"}
					<textarea rows = "{$master_field.rows}" cols="{$master_field.cols}" name="{$master_field.name}"></textarea> 
				{/if}
			</td>
			</tr>
		{/foreach}
		...
		{if $error_msg != ""}
			<input type="button" value="Clear" onclick="clearOnAddForm(this.form);"/>
		{else}
			<input type="reset" value="Clear" onclick="clearOnAddForm(this.form);"/>
		{/if}
			</td>
			</tr>
		</table>
