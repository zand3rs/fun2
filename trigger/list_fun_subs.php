...
//new
$master =& new View_Fun_Subscribers();
...
{
	$gSmarty->assign('error_msg', INVALID_OFW_SIM);
}
else if( (strlen($q_linkto) > 0) && !@preg_match("/^639[0-9]{9}$/i", $q_linkto) )
{
	$gSmarty->assign('error_msg', INVALID_LOCAL_SIM);
}
else
{
	//get list
	$listdata  = $master->ListSubscribers($extravars, $q_sdate, $q_edate, $q_tran_id, $q_status, $q_msisdn, $q_linkto, $q_service);
