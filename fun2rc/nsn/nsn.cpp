...
int NSN::activate(const char *msisdn)
{
    ...
    char ora_auth[256];
    snprintf(ora_auth, sizeof(ora_auth), "%s/%s@%s", _param.db_user, _param.db_pass, _param.db_host);
    ...
    if (conn.initialize(ora_auth) < 0) {
        LOG_ERROR("%s::%s: Unable to connect to oracle: ora_auth: %s", __class__, __func__, ora_auth);
        return -1;
    }
