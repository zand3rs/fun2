...
begin
   ...
   if nvl(p_ref_id, 0) > 0 then
      begin
         select subs_name, subs_group, subs_contact_no, remarks, duration, customer_type, min_bal, remarks, request_origin, other_type, breathing_period, cs_username
         into   vSubsName, vSubsGroup, vSubsContactNo, vRemarks, nDuration, nCustomerType, nMinBal, vRemarks, vSource, vOtherType, nSubsBreathingPeriod, vCS_Username  
   ...
   if (p_trantype = 8) then
      ...
      if not bPreAct then
         ...
         begin
            insert into sim_activation (id, msisdn, activation_dt, deact_basis_dt, deactivation_dt, subscriber_name, status, duration, min_bal, remarks, request_origin, daily_bal, last_daily_bal_chk , imsi, customer_type, breathing_period, activation_tm, cs_username)
            values (sim_activation_seq.nextval, p_msisdn, trunc(sysdate), trunc(sysdate), dDeactivateDt, vSubsName, 'ACTIVE', nCustDuration, nCustMinBal, vRemarks, vSource, nDaily_bal, trunc(sysdate), p_extra_i_3, vProfile, nCustBreathingPeriod, sysdate, vCS_Username );
         exception
