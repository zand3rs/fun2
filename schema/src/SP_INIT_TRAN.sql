...
begin
   ...
   if (p_trantype = 8) and (sf_is_in_hlr_range(p_msisdn) = 0) then
      nRetr  := 113;
      p_retr := nRetr;
      return;
   end if;
   ...
   if (p_trantype = 2) and (sf_is_in_hlr_range(p_msisdn) = 0) then
      nRetr  := 117;
      p_retr := nRetr;
      return;
   end if;
   ...
   -- REG - only PREPAID are allowed
   if (p_trantype = 2) and (sf_is_in_hlr_range(p_extra_i_3) = 0) then
      nRetr  := 118;
      p_retr := nRetr;
      return;
   end if;
   ...
   if (p_trantype in (2,4,8,10)) then
      ...
      if nvl(p_ref_id, 0) > 0 then
         begin
            select nvl(min_bal,0), nvl(duration,0), tx_date, nvl(breathing_period,0)
            into   nMinbal, nDuration, dActivation, nBreathing_period
      ...
      -- GROAM ON 
      if (p_trantype = 8) and (sf_is_roamer_info(p_msisdn, dSubsActDt, dSubsDeactDt, nSubsHotDur, nSubsInHot, dSubsHotDt, vStatus ) > 0) then
         ...
         if (nBreathing_period=0) and (nSubsHotDur>0) and (nSubsInHot > 0) and
            ((nvl(dSubsHotDt,dSubsActDt)-dSubsActDt) >= nSubsHotDur) and 
      ...
      if (p_trantype = 10) and (sf_is_active_roamer(p_msisdn) = 0) then
         nRetr := 102;
         p_retr := nRetr;
         sp_logger('INIT' , 'END => p_trantype :' || to_char(p_trantype) || ' p_retr:' || to_char(p_retr) || ' p_extra_o_1: ' || p_extra_o_1 || ', p_extra_o_2:' || p_extra_o_2 || ', p_extra_o_3:' || p_extra_o_3);
         return;
      ...
      if (p_trantype = 10) then
         if (nSubsInHot > 0) then
            ...
            if (nDuration is null) or (nDuration=0) or (nCustDuration < nDuration) then
               nRetr := 111;
               p_retr := nRetr;
               sp_logger('INIT' , 'END => p_trantype :' || to_char(p_trantype) || ' p_retr:' || to_char(p_retr) || ' spcl nCustDuration: ' || to_char(nCustDuration) || ', nDuration:' || to_char(nDuration) );
               return;
            end if;
         else
            -- allow open duration for subs in non-hot country
            ...
            if (nDuration is not null) and (nDuration > 0 ) and (nCustDuration < nDuration) then
               nRetr := 111;
               p_retr := nRetr;
               sp_logger('INIT' , 'END => p_trantype :' || to_char(p_trantype) || ' p_retr:' || to_char(p_retr) || ' rgrl nCustDuration: ' || to_char(nCustDuration) || ', nDuration:' || to_char(nDuration) );
               return;
            end if;
         end if;
      end if;
      ...
      if (dActivation > trunc(sysdate)) and (p_trantype = 8) then
         ...
         exception
            ...
            when others then
              nRetr := SQLCODE;
              p_retr := nRetr;
              sp_logger('INIT' , 'END => p_trantype :' || to_char(p_trantype) || ' ORAERR p_retr:' || to_char(p_retr) || ' p_extra_o_1: ' || p_extra_o_1 || ', p_extra_o_2:' || p_extra_o_2 || ', p_extra_o_3:' || p_extra_o_3);
              return;
         end;
   ...
   -- 6 - TRAN_TYPE_CANCEL
      ...
      return;
   -- 14 - TRAN_TYPE_YES
   elsif (p_trantype = 14) and (sf_is_link_pending(p_msisdn) = 0) then
      nRetr := 141;
      p_retr := nRetr;
      sp_logger('INIT' , 'END => p_trantype :' || to_char(p_trantype) || ' p_retr:' || to_char(p_retr) || ' p_extra_o_1: ' || p_extra_o_1 || ', p_extra_o_2:' || p_extra_o_2 || ', p_extra_o_3:' || p_extra_o_3);
      return;
