...
int main (int argc, char *argv[])
{
    ...
    while (0 == csv->getline()) {
        ...
        memset(&geo_probe, 0, sizeof(geo_probe_t)); 

        //-- set cluster_node (default 1 for now...)
        geo_probe.cluster_node = 1;

        char imsi[32];
        snprintf(imsi, sizeof(imsi), "%s", csv->getfield(35));
        snprintf(geo_probe.imsi, sizeof(geo_probe.imsi), "%s", (! strncasecmp("0x", imsi, 2)) ? &imsi[2] : imsi);
        snprintf(geo_probe.sccp, sizeof(geo_probe.sccp), "%s", csv->getfield(8));

        LOG_DEBUG("%s: imsi: %s, sccp: %s", app_name, geo_probe.imsi, geo_probe.sccp);

        if (conn.insertGeoProbe(&geo_probe) < 0) {
            LOG_ERROR("%s: Unable to insert geo_probe imsi: %s, sccp: %s", app_name, geo_probe.imsi, geo_probe.sccp);
        }
    }
