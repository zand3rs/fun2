...
int OraDB::init_lib(bool threaded_mode)
{
    ...
    if (threaded_mode) {
        ...
        if (SQLO_SUCCESS != sqlo_init(SQLO_OFF, 1000, 10000)) {
            LOG_CRITICAL("%s: Failed to init libsqlora!!!", __func__);
            return -1;
        }
    }
...
int OraDB::ora_reconnect() 
{
    ...
    _reconnected = (status < 0) ? false : true;
    return status;
}
...
int OraDB::ora_force_execute (sqlo_stmt_handle_t* psth, int check_res, int commit) {
    ...
    for(;;) {
        ...
        if(oci_status == SQLO_SUCCESS && _result == 500) {
            ...
            //LOG_DEBUG("%s: oci_status = SQLO_NO_DATA. STATEMENT: \"%s\"", __func__, sqlo_command(*psth));
            _result = 0;
            break;
        }
        else if (oci_status == SQLO_SUCCESS && _result != 500) {
            //LOG_DEBUG("%s: oci_status = SQLO_SUCCESS. STATEMENT: \"%s\"", __func__, sqlo_command(*psth));
            break;
        }
